  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="glass-card !p-3 border border-[rgba(255,255,255,0.5)] dark:border-[rgba(255,255,255,0.1)] rounded-xl shadow-xl text-xs min-w-[150px]">
        <p className="text-text-muted dark:text-white font-bold mb-2 border-b border-black/10 dark:border-white/10 pb-1">{label}</p>
        <p className="flex justify-between gap-4 mb-1.5">
          <span className="text-text-main dark:text-gray-300 font-medium">Entrées (Ventes) :</span>
          <span className="font-bold text-primary dark:text-[#34d399]">{fmt(ca)} {devise}</span>
        </p>
        <p className="flex justify-between gap-4 mb-2">
          <span className="text-text-main dark:text-gray-300 font-medium">Sorties (Dépenses) :</span>
          <span className="font-bold text-red-600 dark:text-red-400">{fmt(depenses)} {devise}</span>
        </p>
        <div className="border-t border-black/10 dark:border-white/10 pt-2 flex justify-between gap-4">
          <span className="text-text-main dark:text-white font-bold">Bénéfice Réel :</span>
          <span className={`font-bold ${benefice >= 0 ? 'text-primary dark:text-[#34d399]' : 'text-red-600 dark:text-red-400'}`}>
            {fmt(benefice)} {devise}
          </span>
        </div>
      </div>
      ...
      <div className="mt-6 mb-6">
        ...
        {phraseSante && (
          <div className={`mt-2.5 p-3 rounded-xl border text-xs font-bold flex items-center justify-between transition-all shadow-sm ${
            isPositiveHealth === true
              ? 'bg-[#059669]/10 border-[#059669]/30 text-[#047857] dark:text-[#34d399]'
              : isPositiveHealth === false
